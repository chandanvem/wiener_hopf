...
!! whether to compute velocity potential or pressure:
    ...
    if (vortswitch == 1) then
       
       call incvort

       print*,''
       print*,'Incident Vorticity mode computed.'
       deallocate(szi,szk,initpoints)

       mup = sp1  !! the incident wave is the instability wave
    ...
    if (vortswitch == 2) then
       ...
       print*,''
       print*,'Incident Supersonic Instability mode computed.'
       deallocate(szi,szk,initpoints)

       mup = spsp(1)  !! the incident wave is the first supersonic  instability wave
...
!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!
!! 1. Computes the incident vorticity subproblem
!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!=!!

    call vort_initialize

    call vort_definecontours

    call vort_precompute


  END SUBROUTINE incvort


  SUBROUTINE vort_initialize
    ...
    if (vortswitch == 1) then
       print*,''
       print*,'Computing the Incident Vorticity mode...'
    end if

    if (vortswitch == 2) then
       print*,''
       print*,'Computing the Incident Supersonic Instability mode...'
    end if
...
!! since we use the residue theorem to compute them anyway:
    ...
    call checkloc(sp1,i1)

!! if any of them lie inside ask to redefine the contour:

    if(i1==0) then
       print*,''
       print*,'Redefine the contour (upstream problem): Vorticity Zero inside'
       print*,''
       STOP
    end if
    ...
    do i1 = 1, Npsp
       ...
       if(i2 == 0) then
          print*,''
          print*,'Redefine the contour (upstream problem): Following Supersonic Zero is inside:'
          print*,spsp(i1)
       end if
    end do
...
!! compute the various contour parameters:
  ...
  END SUBROUTINE vort_initialize


  SUBROUTINE vort_definecontours
...
!! length of each panel (kernel contour):
  ...
  END SUBROUTINE vort_definecontours
...
!! dump data
  ...
  SUBROUTINE vort_precompute

    complex(dpk)        :: alp, k, kp, km, kp1

    if (M1 < 1) then
       if((REAL(mu0) > (1._dpk/(1._dpk + M1))) .OR. (REAL(mu0) < (-1._dpk/(1._dpk - M1)))) then 
          print*,'Radial wave number is imaginary in upstream nozzle!'
          STOP
       end if
    else
       if((REAL(mu0) < (1._dpk/(M1 - 1._dpk))) .AND. (REAL(mu0) > (1._dpk/(M1 + 1._dpk)))) then 
          print*,'Radial wave number is imaginary in upstream nozzle!'
          STOP
       end if
    end if

    alp = wr*SQRT((1._dpk - mu0*M1)**2 - mu0**2)

    psi = bessj(alp*h,circmod,1)*EXP(ABS(AIMAG(alp*h)))

    if (REAL(mu0) < crpt) then  !! muplus is below the contour; crpt being the crossover pt
       print*, ''
       print*, 'The incident acoustic mode (for vorticity) needs to be INSIDE the contour'
       print*, ''
       STOP
    end if

!    call kernel_eval(mu0,k,1,0,0)
    call kernel_eval(mu0,k,1,0,1)

    kp = EXP(-k/(2._dpk*PI*CMPLX(0._dpk,1._dpk,kind=dpk)) + & 
         LOG(kernel(1,mu0)/zp(1,mu0)))

!!$    km =  kernel(1,mu0)/kp
    km =  kernel(1,mu0)/(kp*zp(1,mu0))

    if (vortswitch == 1) then

       call kernel_eval(sp1,k,1,0,1)

       kp1 = EXP(-k/(2._dpk*PI*CMPLX(0._dpk,1._dpk,kind=dpk)))    !! NOTE: zero sp1 has to lie below
                                                               !! the contour
!!$    resp = kap2*psi*(1._dpk - mu0*M1)/((mu0 - sp1)*km*kp1* &
!!$         (sp1-CONJG(sp1)))

       resp = kap2*psi*(1._dpk - mu0*M1)/((mu0 - sp1)*km*kp1)

       
       write(*,'(A15,2F15.5)') 'Vort_wave amp:', resp
       !    print*, 'res:', resp
       open(10,file='vortwave_amp.out',form='FORMATTED')
       write(10,'(A10,2E20.10)') 'Amp:-->',resp
       close(10)
    ...
    if (vortswitch == 2) then
       ...
       call kernel_eval(spsp(1),k,1,0,1)

       kp1 = EXP(-k/(2._dpk*PI*CMPLX(0._dpk,1._dpk,kind=dpk)))    !! NOTE: zero spsp has to lie below
                                                               !! the contour
!!$    resp = kap2*psi*(1._dpk - mu0*M1)/((mu0 - spsp(1))*km*kp1* &
!!$         (spsp(1)-CONJG(spsp(1))))

       resp = kap2*psi*(1._dpk - mu0*M1)/((mu0 - spsp(1))*km*kp1)
       
       write(*,'(A15,2F15.5)') 'Supins_wave amp:', resp
       !    print*, 'res:', resp
       open(10,file='supinswave_amp.out',form='FORMATTED')
       write(10,'(A10,2E20.10)') 'Amp:-->',resp
       close(10)

    end if

  END SUBROUTINE vort_precompute
