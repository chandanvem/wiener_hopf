...
!! the factor \Psi_{mn}(1) of (3.30) [see the JFM]:
       ...
       psi = input_data%kap_rho*f1*f2/f3
...
!!  the factor Kt^{-}(\mu_{mn}^{+}):
    ...
    k_plus_at_mu_plus = EXP(-intgrl_A1_at_mu_plus/(2._dpk*PI*CMPLX(0._dpk,1._dpk,kind=dpk)) + & 
                        LOG(compute_kernel(0,input_data%mu_plus)/compute_U_s_factor(0,input_data%mu_plus)))

    k_minus_at_mu_plus =  compute_kernel(0,input_data%mu_plus)/(k_plus_at_mu_plus*compute_U_s_factor(0,input_data%mu_plus)) 
...
!!  the factor Kt^{+}(s_{z1}):
    ...
    if (input_data%vortswitch .EQ. 0) then
       ...
       k_plus_sz1 = EXP(-intgrl_A1_at_KH_zero_1/(2._dpk*PI*CMPLX(0._dpk,1._dpk,kind=dpk))) 
...
!!  the factor K^{+}(s_{z2}):
    ...
    if (input_data%num_sup_zeros > 0) allocate(kzsp(input_data%num_sup_zeros))

    if (input_data%vortswitch .EQ. 0) then
      do i1 = 1, input_data%num_sup_zeros  
       print '(A, I0)', 'precompute: Evaluating kernel at supersonic zero ', i1
       call compute_eqn_A1_integral(sup_zeros_list(i1),intgrl_at_sup_zero,0,0,1,input_data,contour_data )
